...
const getPointerInput = (callback: (props: any) => any, element: any = document, delay: number = 600) => {
    ...
    // set up the handlers ^.^
    element.addEventListener('touchstart', (e: any) => handlePointer(e), true);
    element.addEventListener('touchmove', (e: any) => handlePointer(e), true);
    element.addEventListener('mousemove', (e: any) => handlePointer(e), true);

    return false;
};

const clearPointers = (element: any = document) => {
    element.removeEventListener('touchstart', () => { }, true);
    element.removeEventListener('touchmove', () => { }, true);
    element.removeEventListener('mousemove', () => { }, true);
}
...
class StarField {
    ...
    container: vec2;
    constructor(howManyStars: number, canvas: any, depth: number = 2, UIFadeDelay = 1) {
        this.canvas = canvas;
        ...
        let handlePointer = (pointer: { x: number; y: number; hasMoved: any; isMoving: any; }) => {
